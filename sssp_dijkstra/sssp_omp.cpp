...
void dijkstra_omp(vector<vector<int>>& graph, int source) {
    ...
    for (int i = 0; i < numVertices - 1; ++i) {
        // Find the vertex with the minimum distance in parallel
        ...
        // Update the distances of the adjacent vertices in parallel
        #pragma omp parallel for
        for (int j = 0; j < numVertices; ++j) {
            if (!visited[j] && graph[minVertex][j] != 0 &&
                ...
                #pragma omp critical
                {
                    if (distances[minVertex] + graph[minVertex][j] < distances[j]) {
